...
export default function Sidebar({
  ...
  return (
    <nav className="hidden md:flex flex-col fixed left-0 top-16 h-[calc(100vh-64px)] w-64 p-4 bg-slate-50 dark:bg-slate-950 border-r border-slate-200 dark:border-slate-800 transition-colors z-40">
      ...
      {/* System Status Block */}
      <div className="px-3 py-4 border-b border-slate-200 dark:border-slate-800 mb-4 bg-white/50 dark:bg-slate-900/30 rounded-xl">
        <p className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest">
          System Status
        </p>
        <div className="mt-1.5 flex items-center gap-2 text-status-eco">
          <span className="relative flex h-2.5 w-2.5">
            <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-status-eco opacity-75"></span>
            <span className="relative inline-flex rounded-full h-2.5 w-2.5 bg-status-eco"></span>
          </span>
          <span className="text-lg font-bold text-slate-800 dark:text-slate-200">Online</span>
        </div>
      </div>
